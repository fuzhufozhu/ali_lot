...
3.  连接说明。 
    ...
    使用WebSocket方式进行连接，区别主要在MQTT连接URL的协议和端口号，MQTT连接参数和TCP直接连接方式完全相同，其中要注意securemode参数，使用wss方式连接时`securemode=2`，使用ws方式连接时`securemode=3`。
    ...
    建议您使用设备端SDK接入物联网平台。如果您自行开发接入，可参见[MQTT连接签名示例](intl.zh-CN/设备端开发指南/使用开放协议自主接入/MQTT连接签名示例.md#)。
