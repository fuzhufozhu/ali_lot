...
2.  创建高级版产品**路灯产品**和对应设备**Lamp**。
3.  进入Lamp的**设备详情**页，查看**运行状态**。

    新建设备的最新属性值和期望属性值都为null，期望属性值版本为0。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/134519/155192469339879_zh-CN.png)

4.  记录Lamp的设备证书（ProductKey、DeviceName、DeviceSecret）和地域（RegionId）信息。

## 云端开发 {#section_jpc_cg2_zgb .section}

云端通过调用API，获取设备最新属性值，设置期望属性值控制设备。

具体操作可参考[云端API](../../../../../intl.zh-CN/云端开发指南/云端API参考/概述.md#)文档，这里以[Java SDK（云端）](../../../../../intl.zh-CN/云端开发指南/云端SDK参考/Java SDK使用说明.md#)为例。

-   调用QueryDeviceDesiredProperty查看设备的期望属性值。

    ```
    DefaultProfile profile = DefaultProfile.getProfile(
            "<your-region-id>",          // 地域ID
            "<your-access-key-id>",      /阿里云AM账号的AccessKey ID
            "<your-access-key-secret>"); 阿里云RAM账号Access Key Secret
...
-   调用SetDeviceDesiredProperty设置期望属性值。

    ```
    DefaultProfile profile = DefaultProfile.getProfile(
            "<your-region-id>",          // 地域ID
            "<your-access-key-id>",     /  /阿AM账号的AccessKey ID
            "<your-access-key-secret>"); 阿里云RAM账号Access Key Secret
    IAcsClient client = new DefaultAcsClient(profile);
...
灯泡开灯时，主动获取云端期望属性值。灯泡运行过程中云端期望属性值发生改变时，接收云端的实时推送。
...
2.  加入方法，用于变更实际灯泡的属性，并在属性变更后，主动将信息上报到最新属性值中。

    ```
    /**
     * 真实设备处理属性变更在以下，两个场下会被调用：
     * 场景1. 连备联网后主动获取最新的属性期望值（由设备发起，拉模式）
     * 场景2. 设时过程中接收到云端property.set推送（由云端发起，推模式）
...
3.  灯泡在线时，如果云端设置了灯的期望属性值，该值将被推送到设备端。灯处理消息，改变状态。
    ...
    灯启动后，参考`connectNotifyListener`方法注册处理消息。相关Alink协议请参考[设备上报属性](../../../../../intl.zh-CN/设备端开发指南/基于Alink协议开发/设备属性、事件、服务.md#section_g4j_5zg_12b)章节。
...
-   设备在线时，服务端修改灯泡开关状态，灯泡实时响应状态变化。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/134519/155192469339883_zh-CN.png)

-   设备不在线时，服务端修改灯泡开关状态，云端期望属性值与设备的最新属性值出现不一致。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/134519/155192469339884_zh-CN.png)

-   设备联网后，设备主动拉取期望属性值，设备的最新属性值实现与云端期望属性值的同步。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/134519/155192469339885_zh-CN.png)


总结：只需修改期望属性值即可改变灯泡的开关状态。没有存储能力的灯泡，通过期望属性值实现了灯泡开关状态的控制。

## 设备端Demo代码 {#section_qyj_wb2_zgb .section}
...
public class LampDemo {
     ...
     * 场景2. 设备在线过程中接收到云端property.set推送（由云端发起，推模式）
     * @param identifier  属性标识符
     * @param value       期望属性值
     * @param needReport  是否发送property.post状态上报。
     *                    上面场景2的处理函数中已集成属性上报能力，会将needReport设置为false
