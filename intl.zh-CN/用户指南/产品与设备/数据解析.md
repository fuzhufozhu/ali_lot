...
-   Alink method不存在。
    ...
    日志中显示错误码为6450。[日志说明文档](intl.zh-CN/用户指南/监控运维/日志服务.md#)中有该错误码解释：错误码6450表示Alink协议格式的数据中method不存在。 原因是设备上报的Alink标准格式数据，或者自定义/透传格式数据，经过脚本解析为Alink标准格式数据后无method。
